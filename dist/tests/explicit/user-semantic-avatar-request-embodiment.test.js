...
export const CONTROL_POINT = 'scripts/control-avatar.mjs --intent "show a thinking state" --request-id TC-EX-004';
export const OBSERVATION_POINT = "The semantic control result exposes action 'thinking' and structured Blender feedback with an acknowledged thinking-compatible state.";
test(`${TESTCASE_ID} UserSemanticAvatarRequestEmbodiment`, async (t) => {
    ...
    assert.equal(result.feedback?.observedState, baseline.expectedAction, `Observation point: ${OBSERVATION_POINT}`);
