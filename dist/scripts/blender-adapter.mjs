...
const pythonDriver = resolve(workspaceRoot, "scripts", "blender-avatar-driver.py");
const tempDir = await mkdtemp(join(tmpdir(), "auto-skepture-blender-"));
...
try {
    ...
    const { stderr } = await runProcess(blenderExecutable, args, {
        ...
        timeoutMs: parseTimeout(process.env.BLENDER_ADAPTER_TIMEOUT_MS) ?? 30_000,
    });
...
async function tryResolveViaWindowsRegistry() {
    ...
    try {
        const powershellPath = process.env.SystemRoot !== undefined
            ...
            : "powershell.exe";
        const { stdout } = await runProcess(powershellPath, ["-NoProfile", "-Command", script], { timeoutMs: 10_000 });
...
function runProcess(command, args, options) {
    return new Promise((resolvePromise, reject) => {
        ...
        child.on("close", (code) => {
            finish(() => {
                if (code !== 0) {
                    reject(new Error(`Blender exited with code ${code}. ${stderr.trim() || stdout.trim() || "No output."}`));
                    return;
                }
