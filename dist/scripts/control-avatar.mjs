...
import { getSupportedAvatarActions, resolveAvatarIntent, } from "./avatar-intent-mapping.mjs";
...
if (!ACTIONS.has(resolvedAction)) {
    console.error(`Unsupported action '${resolvedAction}'. Allowed actions: ${[...ACTIONS].join(", ")}.`);
    process.exit(1);
}
try {
    ...
    process.stdout.write(`${JSON.stringify({ action: resolvedAction, ...result }, null, 2)}\n`);
}
catch (error) {
...
function runRuntimeHelper(action, requestId) {
    return new Promise((resolvePromise, reject) => {
        ...
        const helperEnv = {
            ...process.env,
            AUTO_SKEPTURE_USE_REPO_ADAPTER: process.env.AUTO_SKEPTURE_USE_REPO_ADAPTER ??
