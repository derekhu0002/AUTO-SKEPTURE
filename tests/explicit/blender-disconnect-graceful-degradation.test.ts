...
test(`${TESTCASE_ID} BlenderDisconnectGracefulDegradation`, async (t) => {
  if (!shouldRunRealEnvironmentDisconnectTest()) {
    t.skip(
      "Real disconnect environment is not configured. Set BLENDER_ADAPTER_DISCONNECT_COMMAND or BLENDER_ADAPTER_COMMAND.",
    );
    return;
  }

  const baseline = explicitBaselines.gracefulDegradation;
  const mediator = new AvatarEmbodimentMediator(
    new StdioBlenderBridge(getRealEnvironmentDisconnectBridgeOptions()),
  );
  ...
  assert.match(
    outcome.surfacedStatus,
    /Avatar degraded: Blender adapter unavailable/,
