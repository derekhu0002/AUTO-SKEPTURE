...
test("user semantic entrypoint guard: low-level rig requests are rejected", async () => {
  const controlPoint =
    'Invoke scripts/control-avatar.mjs --intent "rotate the right hand bone 30 degrees".';
  const observationPoint =
    "The semantic control entrypoint exits non-zero and rejects low-level rig control instead of coercing the request into Blender automation.";
