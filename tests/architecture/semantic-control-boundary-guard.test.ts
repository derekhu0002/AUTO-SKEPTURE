...
test("semantic control boundary guard: host semantic control assets do not launch the bridge adapter directly", async () => {
  ...
  const observationPoint =
    "The control asset delegates semantic execution through the runtime/mediator boundary instead of launching scripts/blender-adapter.mjs directly.";
  const controlAsset = await readFile(
    new URL("../../scripts/control-avatar.mjs", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    controlAsset,
    /AgentRuntime|AvatarEmbodimentMediator/,
