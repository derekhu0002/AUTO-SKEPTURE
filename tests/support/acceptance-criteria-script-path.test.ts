...
test("support guardrail: explicit testcase acceptanceCriteria point to executable scripts", async () => {
  const controlPoint =
    "Read explicit testcase acceptanceCriteria values from design/KG/SystemArchitecture.json.";
  const observationPoint =
    "Each explicit testcase points to an existing single executable script file accepted by the failure-record consumer.";
