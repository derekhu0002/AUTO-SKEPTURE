...
const CONTROL_POINT =
  "Set BLENDER_LIVE_PORT without BLENDER_ADAPTER_COMMAND and resolve bridge options.";
const OBSERVATION_POINT =
  "The bridge resolves to the repository-local live adapter command so control requests can target an already-open Blender instance through the existing bridge boundary.";
...
test("support guardrail: bridge options can target a live Blender control server", () => {
  ...
  try {
    ...
    assert.match(
      options.args?.[0] ?? "",
      /scripts[\\/]blender-live-adapter\.mjs$/,
